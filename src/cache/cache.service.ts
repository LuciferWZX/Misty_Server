...
export class CacheService {
  ...
  public async hGet(key: RedisKey, fieldKey: string): Promise<any> {
    ...
    const data = await this.client.hget(key, fieldKey);
    return JSON.parse(data);
  }
   ...
   * @param key
   */
  public async hGetAll(key: RedisKey): Promise<any> {
    if (!this.client) {
      await this.getClient();
    }
   ...
   * @param value
   */
  public async hSet(key: RedisKey, fieldKey: string, value: any): Promise<any> {
    if (!this.client) {
      await this.getClient();
    }
    return await this.client.hset(key, fieldKey, JSON.stringify(value));
  }
  /**
   * @desc 设置过期时间
   * @param key {RedisKey}
